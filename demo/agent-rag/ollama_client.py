...
GROQ_BASE_URL = os.environ.get("GROQ_BASE_URL") or _cfg.get("GROQ_BASE_URL", "https://api.groq.com/openai/v1")
GROQ_MODEL    = os.environ.get("GROQ_MODEL")    or _cfg.get("GROQ_MODEL", "llama-3.3-70b-versatile")
OLLAMA_URL    = os.environ.get("OLLAMA_URL")    or _cfg.get("OLLAMA_URL", "http://localhost:11434")
OLLAMA_MODEL  = os.environ.get("OLLAMA_MODEL")  or _cfg.get("OLLAMA_MODEL", "mistral-nemo:latest")
...
def _sanitize_schema(schema: dict) -> dict:
    """
    Sanitize MCP tool schema for Groq compatibility.
    Groq rejects anyOf/null patterns — replace with simple types.
    Also removes 'title' fields which are unnecessary noise.
    """
    ...
    for key, value in schema.items():
        if key == "title":
            continue  # Groq doesn't need titles
        elif key == "anyOf" and isinstance(value, list):
            # anyOf: [{"type": "X"}, {"type": "null"}] -> just use type X
            non_null = [v for v in value if v.get("type") != "null"]
            if len(non_null) == 1:
                # Merge the non-null type into parent
                result.update(_sanitize_schema(non_null[0]))
            else:
                result[key] = [_sanitize_schema(v) for v in value]
